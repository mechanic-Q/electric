...
- [ ] task-01: 新增 PyTorch DNN 电价预测器（覆盖：FR-01, D-001@v1, D-003@v1）
- [ ] task-02: 新增 DNN forecaster 单元测试（覆盖：FR-08）

## Wave 2（comparison script）

- [ ] task-03: 新增 `compare_price_models.py` 脚本骨架与统一数据切分（覆盖：FR-02, FR-03, D-002@v1）
- [ ] task-04: 实现 LEAR/DNN/persistence/weekly_avg 统一评估（覆盖：FR-02, FR-03, FR-04）
- [ ] task-05: 接入 DM/GW 统计检验（覆盖：FR-05）

## Wave 3（reports/integration）

- [ ] task-06: 生成 JSON/MD/HTML/log 报告（覆盖：FR-06, FR-07）
- [ ] task-07: 可选扩展 service/CLI 支持 price DNN model_type（覆盖：D-004@v1）
- [ ] task-08: 更新模块文档（覆盖：D-001@v1, D-002@v1）
