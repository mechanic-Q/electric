...
## 契约摘要
- `persistence_forecast(df)` — 持续法基线：`forecast[t] = load_mw[t-24h]`，前 `TimeConfig.points_per_day` 点 bfill 补齐
...
- `XGBoostForecaster` 类：
  - `train_evaluate(X, y, n_splits=5, gap=TimeConfig.points_per_day)` — TimeSeriesSplit, 每 fold 内 StandardScaler fit-on-train-only, XGBRegressor, 返回 `{predictions, actuals, metrics: {mae}, model, feature_importance}`
  - `predict(X)` — 推理
  - `save_model(path)` / `load_model(path)` — joblib 持久化
  - `plot_forecast(df, predictions, title)` — 双子图 plotly（实际vs预测叠加 + 误差分布直方图）
## 关键逻辑
```
forecast = df["load_mw"].shift(TimeConfig.points_per_day).bfill()  # → 持续法
for train_idx, test_idx in tscv.split(X):          # gap=96 防 lag 泄露
