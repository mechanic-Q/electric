...
- **Global scope**: `/mnt/e/Ellectric/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/mnt/e/Ellectric/.opencode/notes/`)
...
2. Glob `/mnt/e/Ellectric/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (/mnt/e/Ellectric/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `/mnt/e/Ellectric/.opencode/notes/` — works in any directory
