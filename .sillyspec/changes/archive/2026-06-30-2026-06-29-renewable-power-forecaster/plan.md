...
- [x] task-01: 新增 `renewable_forecaster.py` 基础结构与共享训练评估逻辑（覆盖：FR-01, FR-02, FR-03, D-001@v1, D-003@v1）
- [x] task-02: 实现 Tier1-4 特征构建与缺 weather 降级（覆盖：FR-04, D-004@v1）
- [x] task-03: 新增 renewable forecaster 单元测试骨架与 fake data（覆盖：FR-08）

## Wave 2（integration）

- [x] task-04: 扩展 service schema/handler 支持 `model_type=wind|solar`（覆盖：FR-06）
- [x] task-05: 扩展 API/CLI/LLM forecast 查询入口（覆盖：FR-06）

## Wave 3（validation/report）

- [x] task-06: 新增 `validate_renewable_forecaster.py` 与 JSON/Markdown/log 报告产物（覆盖：FR-05, FR-07）
- [x] task-07: 新增模块文档 `renewable-forecaster.md`（覆盖：D-002@v1）
