...
- FR-003: 气象数据与山东 15min `timestamp` 对齐，天气列可进入 `get_feature_columns("tier4")` 与 `prepare_features(tiers=[..., "tier4"])`。
...
兼容策略：`tiers` 不包含 `"tier4"` 时保持当前行为；旧调用者不受影响。
...
- Tier1-3 默认行为不变。
- `prepare_features()` 原有 `df, tiers` 调用仍可用；新增参数均有默认值。
