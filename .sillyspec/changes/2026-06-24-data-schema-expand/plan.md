...
### Wave 1 (同文件串行追加)
- [ ] task-01: 追加 ShanxiMonthDealLoader（覆盖：FR-001, D-001@v1, D-002@v1）
- [ ] task-02: 追加 ShanxiUserTransactionLoader（覆盖：FR-002, D-001@v1, D-002@v1）
- [ ] task-03: 追加 ShanxiYearTradeFitLoader（覆盖：FR-003, D-001@v1, D-002@v1）
- [ ] task-04: 追加 ShanxiMonthSettle1Loader（覆盖：FR-004, D-001@v1）
- [ ] task-05: 追加 ShanxiTimeDivTrendLoader（覆盖：FR-005, D-001@v1, D-002@v1）

> ⚠️ task-01~05 物理上修改同一文件 `shanxi_loader.py`，必须串行追加

### Wave 2 (依赖全部子类)
- [ ] task-06: data_loader.py create_loader 加 5 elif 分支（覆盖：FR-006）

### Wave 3 (依赖工厂扩展)
- [ ] task-07: verify_shanxi_loader.py 扩展 5 source 验证段（覆盖：FR-007, D-003@v1）

## 验收

- [ ] `python ellectric/scripts/verify_shanxi_loader.py` 退出码 0
- [ ] 总验证 ≥ 35 项
- [ ] 5 个新 source 全部 create_loader().load_data() 返回非空
- [ ] 现有 24 项验证（spot_da/spot_rt/month_settle/优雅降级/兼容性）全通过
- [ ] shanxi_loader.py 既有 4 类源码零修改
- [ ] requirements.txt 不变
- [ ] data_loader.py 顶层无 shanxi_loader import（延迟导入）
